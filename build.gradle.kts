...
java {
    ...
    // sourceCompatibility/targetCompatibility above only set javac's target
    // bytecode level - without this, Gradle still compiles and runs every
    // task (including runClient) on whichever JDK the Gradle daemon itself
    // started with, regardless of javaVersion. That's fine for loaders that
    // don't care, but old Forge's mods.toml parsing breaks in strange ways
    // (e.g. InvalidModFileException: Missing required field mandatory) when
    // run under a JDK many majors newer than the one it was built against.
